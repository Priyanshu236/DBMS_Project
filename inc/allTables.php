...
//************************************************* blog_posts  **********************************************
	...
	{	
		...
		if($row1hello<1)
		{
			$hellopost = "INSERT INTO `blog_posts` (`id`, `postTitle`, `description`, `content`, `post_date`, `author`, `catinfo`,`imgUrl`) VALUES (NULL, 'Hello World', 'This is a Sample Post for saying blog setup is successfully completed, you good to go for blogging!', 'Good luck!', CURRENT_TIMESTAMP, 'You', '1',NULL)";
...
//************************************************** blog cats ****************************************************
	...
	{	
		...
		if($rownum<1)
		{
			$q1 = "INSERT INTO blog_cats (catID, catTitle, catDesc) VALUES (NULL, 'Uncategorised', 'uncategorised')";
			mysqli_query($con,$q1);
    ...
    //----------------------Comment---------------------------//
    $verify4 = "SELECT * FROM comments";
	$resverify4 = mysqli_query($con,$verify4);
	...
	{	
		...
		if($rownum<1)
		{
			$q1 = "INSERT INTO comments (C_ID,C_User, C_Desc, C_Date) VALUES (NULL,'check', 'check', CURRENT_TIMESTAMP)";
			mysqli_query($con,$q1);
			$rows1 = mysqli_affected_rows($con);
			if($rows1==1)
			{
			...
			{
				echo 'error while inserting Comment record';
				die();
			}	
		}
	...
	else
	{
		...
		if($result4)
		{
			...
			if($rowsz<1)
			{
				$q1 = "INSERT INTO comments (C_ID,C_User, C_Desc, c_Date) VALUES (NULL,'check', 'Uncategorised', CURRENT_TIMESTAMP)";
				mysqli_query($con,$q1);
				$rows1 = mysqli_affected_rows($con);
				if($rows1==1)
				{
		...
		{
			echo 'error creating table Comment';
			die();
		}
