...
?>	
	...
	<div class="container-fluid topspace">
			<?php
				...
				$query = "SELECT id, postTitle, description, content, post_date, author, catinfo,imgUrl FROM blog_posts WHERE id = '$id'";
				...
				if ($result && mysqli_num_rows($result) > 0)
				{
					...
					{
						echo '<div class="container" style="margin: 20px"><div id="bigpost">';
			                echo '<h1  id="posttitle">'.$row['postTitle'].'</h1>';
			                echo '<p id="postdate"><i>Posted on '.date('jS M Y H:i:s', strtotime($row['post_date'])).'</i> by '.$row['author'];
			...
			/////////////////////////////////// Categoty Code end //////////////////////////////////////////////////////
			                ...
			                echo '<p style="margin: 50px"><h4 id="description1">'.$row['description'].'</h4></p>';
			                echo '<p style="margin: 40px"><h3 id="content">'.$row['content'].'</h3></p>';                           
			            echo '</div></div>';
			            ?>
